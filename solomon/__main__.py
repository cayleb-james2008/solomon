...
def main():
    ...
    parser.add_argument(
        "command",
        choices=["run", "dashboard", "test-browser", "test-vlm", "test-llm", "guard-check", "webhook", "loop"],
        help="run=CEO cycle, dashboard=status, test-*=self-test, guard-check=verify money guard, webhook=Polar listener, loop=continuous CEO",
    )
