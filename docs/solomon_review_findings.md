...
**Deferred (documented for a follow-up session â€” higher blast-radius / lower urgency):**
- **PID-reuse identity token** (HIGH): `_pid_alive` only checks PID existence; a reused PID can pin a
  ...
  (PID + creation-time, or a heartbeat run-id) cross-checked in is_running/clear_lock/acquire_lock â€”
...
- **gate_red_streak anti-thrash ceiling** (MEDIUM): a fix-session can respawn each sweep under
  auto_ai_fix+unattended (the unattended watchdog uses allow_pi=False, so it escalates â€” not affected).
